...
module StateSizedIO.writingOOsUsingIOVers3 where
...
module _ {I : Interfaceˢ } (let S = Stateˢ I) (let n = nextˢ I)
  ...
  translateˢ' {A} {sbegin} {send} obj (execˢ''' c p) = obj .objectMethod c ▹ λ {(x , o')
...
module _ (I₁ : IOInterfaceˢ )
  ...
  mutual
    ...
    translateˢIndCoind+ : ∀{i} → {A : S₁ → S₂ → Set}{s₁ : S₁}{s₂ : S₂}
                 ...
                 → IOˢindcoind+ I₁ I₂' i A s₁ s₂
                 → IOˢ' I₁ i ((λ s₁ → Σ[ s₂ ∈ S₂ ]  (A s₁ s₂ × Objectˢ I₂ s₂))) s₁
    translateˢIndCoind+ {i} {A} {s₁} {s₂} obj (execˢ'' c f) = obj .objectMethod c ▹ λ { (x , o') →
...
module _ {objInf : Interfaceˢ}
        ...
        run+_startingWith_ : IOˢindcoind+ ConsoleInterfaceˢ objIOInf ∞ (λ x y → Unit) unit objState
              → Objectˢ objInf objState
              → NativeIO Unit
        run+ prog startingWith obj = translateIOConsole (flatternIOˢ consoleI (λ c → c) (λ r → r) (fmapˢ ∞  (λ x y → unit) unit (delayˢ (translateˢIndCoind+ ConsoleInterfaceˢ  (ioInterfToObjectInterfˢ objIOInf) obj prog))))
---
