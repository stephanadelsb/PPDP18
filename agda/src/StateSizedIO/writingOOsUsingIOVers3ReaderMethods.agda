...
module StateSizedIO.writingOOsUsingIOVers3ReaderMethods where
...
module _ (I : IOInterfaceˢ {lzero} )
  ...
  mutual
    ...
    data IOˢind (A : S → Set) (s : S) : Set where
      ...
      execˢ''     : (c : C s) (f : (r : R s c) → IOˢind A (n s c r)) → IOˢind A s
...
module _ {I : RInterfaceˢ } (let S = Stateˢ I) (let n = nextˢ I)
  ...
  translateˢ {A} {s} obj (execˢ'' (inj₁ c) p) = obj .objectMethod c ▹ λ {(x , o')
                                      → translateˢ {A} {n s c x} o' (p x)  }
  translateˢ {A} {s} obj (execˢ'' (inj₂ c) p) = obj .readerMethod c ▹ λ x                                                → translateˢ {A} {s} obj (p x)
...
module _ {I : RInterfaceˢ } (let S = Stateˢ I) (let n = nextˢ I)
  ...
  translateˢ' {A} {sbegin} {send} obj (execˢ''' (inj₁ c) p) =
                   obj .objectMethod c ▹ λ {(x , o')
...
translateMultiˢ J I A f obj (execˢ'' (j , m) p) = (obj j) .objectMethod m ▹ λ {(r , o')
...
module _ (I₁ : IOInterfaceˢ )
  ...
  mutual
    ...
    {- IOˢindcoind  to IOˢindcoind∞
       IOˢindcoind+  to IOˢindcoindobj
       IOˢindcoindShape to IOˢindcoindIO
       execˢ'' -> execˢobj
       returnˢ'' -> returnˢobj
       execˢic -> execˢIO
       returnˢic -> returnˢIO
-}
    ...
    data IOˢindcoind+ (i : Size)(A : S₁ → S₂ → Set) (s₁ : S₁)(s₂ : S₂) : Set where
       execˢ'' : (c : C₂ s₂)(f : (r : R₂ s₂ c) → IOˢindcoind+ i A s₁ (n₂ s₂ c r))
              → IOˢindcoind+ i A s₁ s₂
       returnˢ'' : IOˢindcoindShape i A s₁ s₂ → IOˢindcoind+ i A s₁ s₂

-- IOˢindcoind+ i A s₁ s₂ = IOˢind I₂ (IOˢindcoindShape i A s₁) s₂


    data IOˢindcoindShape (i : Size)(A : S₁ → S₂ → Set) : S₁ → S₂ → Set where
      execˢic : {s₁ : S₁} → {s₂ : S₂} → (c₁ : C₁ s₁)
...
{-
    ...
    translateˢ {A} {s} obj (returnˢ'' x) = s , (x , obj)
    translateˢ {A} {s} obj (execˢ'' c p) = obj .objectMethod c ▹ λ {(x , o')
                                        → translateˢ {A} {n s c x} o' (p x)  }
-}
...
module _ {I : IOInterfaceˢ } (let S = Stateˢ I)
   ...
   _>>=ind_ : {A :  S → Set}{B : S → Set}{s : S}
              ...
              → IOˢind I B s
   _>>=ind_  (execˢ'' c f₁) f = execˢ'' c (λ r → f₁ r >>=ind f)
...
module _ (I₁ : IOInterfaceˢ )
   ...
   mutual
     ...
     _>>=indcoind+_  (execˢ'' c f₁) f = execˢ'' c (λ r → f₁ r >>=indcoind+ f)
     _>>=indcoind+_ {s₁ = s₁} {s₂} (returnˢ'' p ) f = p >>=indcoindShape f
     ...
     _>>=indcoindShape_ : {i : Size}{A : S₁ → S₂ → Set}
                      ...
                      (f : (s₁' : S₁)(s₂' : S₂)(a : A s₁' s₂') → IOˢindcoind+ I₁ I₂ i B s₁' s₂')
                      → IOˢindcoind+ I₁ I₂ i B s₁ s₂
     execˢic c₁ f₁ >>=indcoindShape f = returnˢ'' (execˢic c₁ (λ r → f₁ r >>=indcoind f))
     ...
     _>>=indcoindaux'_ : {i : Size}{A : S₁ → S₂ → Set}
                      ...
                      (f : (s₁' : S₁)(s₂' : S₂)(a : A s₁' s₂') → IOˢindcoind+ I₁ I₂ i B s₁' s₂')
                      → IOˢindcoind+ I₁ I₂ i B s₁ s₂
     execˢ' c f₁ >>=indcoindaux' f = returnˢ'' (returnˢic (execˢ' c λ r → {!returnˢic!})) -- execˢ' c λ r → f₁ r >>=indcoindaux f
     _>>=indcoindaux'_ {s₁ = s₁}{s₂ = s₂} (returnˢ' a)  f = f s₁ s₂ a -- execˢ'' {!inj₂ c!} {!!}
...
module _ (I₁ : IOInterfaceˢ )
  ...
  mutual
    ...
    translateˢIndCoind+ : ∀{i} → {A : S₁ → S₂ → Set}{s₁ : S₁}{s₂ : S₂}
                 ...
                 → IOˢindcoind+ I₁ I₂' i A s₁ s₂
                 → IOˢ' I₁ i ((λ s₁ → Σ[ s₂ ∈ S₂ ]  (A s₁ s₂ × RObjectˢ I₂ s₂))) s₁
    translateˢIndCoind+ {i} {A} {s₁} {s₂} obj (execˢ'' (inj₁ c) f) =
                   obj .objectMethod c ▹ λ { (x , o') →
                   translateˢIndCoind+ {i} {A} {s₁} {nextˢ I₂ s₂ c x} o' (f x) }
    translateˢIndCoind+ {i} {A} {s₁} {s₂} obj (execˢ'' (inj₂ c) f) =
                  obj .readerMethod c ▹ λ x  →
                  translateˢIndCoind+ {i} {A} {s₁} {s₂} obj (f x)

    translateˢIndCoind+ {i} {A} {s₁} {s₂} obj (returnˢ'' x) = translateˢIndCoindShape obj x
    ...
    translateˢIndCoindShape : ∀{i} → {A : S₁ → S₂ → Set}{s₁ : S₁}{s₂ : S₂}
                 ...
                 → IOˢ' I₁ i ((λ s₁ → Σ[ s₂ ∈ S₂ ]  (A s₁ s₂ × RObjectˢ I₂ s₂))) s₁
    translateˢIndCoindShape {i} {A} {.s₁} {.s₂} obj (execˢic {s₁} {s₂} c₁ p)
...
callMethod {I₁} {I₂} {A} {s₁} {s₂} c f .forceIC {j} = execˢ'' c  f
...
endIO+ = returnˢ'' (returnˢic (returnˢ' unit)) -- (delayˢic (returnˢic (returnˢ' unit))) -- (returnˢic (returnˢ' unit))
...
execIO+ {i} c p = returnˢ'' (execˢic c p) -- (returnˢic (execˢ' c {!λ r → p r!})) -- (delayˢic (execˢic c (λ r → p r)))
...
execIO : {i : Size}
       ...
       ((r₁ : I₁ .Responseˢ s₁ c₁) →
        IOˢindcoind I₁ I₂ i (λ _ _ → Unit) (nextˢ I₁ s₁ c₁ r₁) s₂)
       → IOˢindcoind I₁ I₂ i (λ _ _ → Unit) s₁ s₂
execIO {i} c p .forceIC {j} = execIO+ c p
...
execIO+' : {i : Size}
       ...
       ((r₁ : I₁ .Responseˢ s₁ c₁) →
        IOˢindcoind I₁ I₂ i (λ _ _ → Unit) (nextˢ I₁ s₁ c₁ r₁) s₂)
       → IOˢindcoind+ I₁ I₂ i (λ _ _ → Unit) s₁ s₂
execIO+' {i} c p = returnˢ'' (execˢic c p) -- (delayˢic (execˢic c (λ r → returnˢ'' (p r)))) -- p r -- (execˢic c ? ) -- )
-}
...
module _ {objInf : RInterfaceˢ}
        ...
        run+_startingWith_ : IOˢindcoind+ ConsoleInterfaceˢ objIOInf ∞ (λ x y → Unit) unit objState
              → RObjectˢ objInf objState
              → NativeIO Unit
        run+ prog startingWith obj = translateIOConsole (flatternIOˢ consoleI (λ c → c) (λ r → r) (fmapˢ {-∞-}  (λ x y → unit) {-unit-} (delayˢ (translateˢIndCoind+ ConsoleInterfaceˢ  objInf obj prog))))
